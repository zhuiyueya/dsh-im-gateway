...
export const feishuProvisioner = {
    async start(callbacks, signal) {
        ...
        const registration = sdk.registerApp({
            ...
            onStatusChange: (info) => {
                ...
                if (status === 'polling' || status === 'slow_down' || status === 'domain_switched')
                    callbacks.onStatus('登录中');
            },
        }).then(async (result) => {
            if (cancelled)
                return;
...
export const dingtalkProvisioner = {
    async start(callbacks, signal) {
        ...
        if (!deviceCode || !verificationUrl)
            throw new Error('钉钉扫码服务未返回完整二维码信息');
        ...
        const task = poll().catch((error) => { if (!cancelled && !signal.aborted)
            callbacks.onFailure(error); });
        return { cancel() { cancelled = true; void task; } };
    },
};
export function classifyWecomQrStatus(value) {
...
export const wecomProvisioner = {
    async start(callbacks, signal) {
        ...
        if (!response.ok)
            throw new Error(`企业微信扫码服务 HTTP ${response.status}`);
        const body = await response.json();
        const scode = cleanString(body?.data?.scode);
        const verificationUrl = cleanString(body?.data?.auth_url);
        if (!scode || !verificationUrl || !verificationUrl.startsWith('https://work.weixin.qq.com/'))
            throw new Error('企业微信扫码服务返回了无效数据');
        ...
        const poll = async () => {
            while (!cancelled && !signal.aborted) {
                ...
                if (!resultResponse.ok)
                    throw new Error(`企业微信扫码轮询 HTTP ${resultResponse.status}`);
                ...
                if (state === 'expired') {
                    callbacks.onFailure(new Error('企业微信二维码已过期'));
                    return;
                }
                if (state === 'failed') {
                    callbacks.onFailure(new Error('企业微信扫码未完成'));
                    return;
                }
                const botId = cleanString(result?.data?.bot_info?.botid);
                const secret = cleanString(result?.data?.bot_info?.secret);
                if (!botId || !secret) {
                    callbacks.onFailure(new Error('企业微信扫码未返回完整机器人凭据'));
                    return;
                }
...
export function provisionerFor(channelId) {
    if (channelId === 'feishu')
        return feishuProvisioner;
    if (channelId === 'qqbot')
        return qqProvisioner;
    if (channelId === 'dingtalk')
        return dingtalkProvisioner;
    if (channelId === 'wecom')
        return wecomProvisioner;
    return undefined;
}
