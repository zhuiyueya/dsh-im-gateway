...
import { provisionerFor } from './core/provisioning.js';
...
export function normalizeChannelStatus(input) {
    ...
    if (FAILED_PROVISIONING.has(provisioning))
        return '异常';
    if (provisioning && !TERMINAL_PROVISIONING.has(provisioning))
        return '连接中';
    if (!input.running)
        return '未连接';
    const raw = input.rawStatus ?? '';
    if (/错误|失败|异常|缺少依赖|已断开|已停止|已登出|仅支持/.test(raw))
        return '异常';
    if (input.loginUrl || /等待|登录中|连接中|重连中|鉴权中|握手|未启动/.test(raw))
        return '连接中';
    return '已连接';
}
export class ChannelManager {
    ...
    async startProvisioning(id) {
        const provisioner = provisionerFor(id);
        if (!provisioner)
            return { ok: false, error: `${CHANNEL_META[id]?.label ?? id} 暂不支持官方扫码接入` };
        await this.cancelProvisioning(id);
        const controller = new AbortController();
        const attempt = { controller, status: '登录中' };
        ...
        this.apiDisposers.push(webServer.register({
            ...
            handler: async (req, res) => {
                ...
                if (parts[2] === 'icon' && parts.length === 4 && req.method === 'GET') {
                    ...
                    const icon = CHANNEL_META[channelId]?.icon;
                    if (!icon) {
