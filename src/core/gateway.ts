...
export class ImGateway {
    ...
    switch (cmd) {
      ...
      case '/status': {
        ...
        if (entry) {
          ...
          if (!title) title = this.titles.get(entry.sessionId) ?? ''
  ...
  private async listSessions(workspace?: string): Promise<string> {
    ...
    const chunks: string[] = ['', `📋 ${workspace ? `会话（当前工作区 ${workspace}）` : '全部会话'}：${filtered.length} 个${filtered.length > 20 ? `，显示前 20` : ''}${workspace ? `（/sessions all 查看全部）` : ''}`]
    if (workspace) {
